...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (!data || data.length === 0) {
            return res.status(404).json({ error: true, message: "No hay registros de USDT en la base de datos." });
        }
        ...
        return res.status(500).json({ error: true, message: "Error USDT: " + e.message });
    }
}
