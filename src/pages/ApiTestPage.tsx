...
const ApiTestPage: React.FC = () => {
    ...
    const baseUrl = "https://konvierte.vercel.app/api";

    const endpoints = [
        { id: 'status', label: 'Estado del Servicio', path: '/status', icon: <Activity size={16} />, desc: 'Verifica la salud y tiempo de actividad de la API.' },
        { id: 'rates', label: 'Tasas de Cambio', path: '/rates', icon: <Database size={16} />, desc: 'Obtiene todas las tasas (BCV + P2P) en un solo objeto JSON.' },
        { id: 'history', label: 'Historial', path: '/history', icon: <Clock size={16} />, desc: 'Consulta los registros históricos de los últimos 30 días.' },
        { id: 'usd', label: 'Dólar BCV', path: '/usd', icon: <DollarSign size={16} />, desc: 'Tasa oficial del Banco Central de Venezuela.' },
        { id: 'eur', label: 'Euro BCV', path: '/eur', icon: <Euro size={16} />, desc: 'Tasa oficial del Banco Central de Venezuela (EUR).' },
        { id: 'usdt', label: 'P2P Binance', path: '/usdt', icon: <Zap size={16} />, desc: 'Promedio representativo del mercado USDT/VES en Binance.' },
    ];
                            ...
                            {activeMenu !== 'team' ? (
                                <motion.div key={activeMenu} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-12">
                                    <section className="space-y-4 text-center sm:text-left">
                                        <h1 className="text-3xl font-extrabold tracking-tight underline underline-offset-8 decoration-emerald-500/20 italic italic">{currentEndpoint?.label}</h1>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className={`flex items-center gap-4 p-5 rounded-2xl border font-mono text-xs border-inherit group relative shadow-md overflow-hidden ${isDark ? colors.codeBg : 'bg-white'}`}>
                                            ...
                                            <span className="truncate opacity-80 font-bold">{baseUrl}{currentEndpoint?.path}</span>
                                            <button onClick={() => copyToClipboard(`${baseUrl}${currentEndpoint?.path}`, 'url')} className="ml-auto p-2 hover:bg-emerald-500/10 rounded-lg hover:text-emerald-600 transition-all opacity-40 group-hover:opacity-100">
                                ...
                                <motion.div key="team" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-16">
                                    ...
                                    <div className={`p-10 rounded-[2.5rem] border flex flex-col sm:flex-row items-center gap-10 shadow-2xl ${colors.card}`}>
                                        ...
                                        <div className="text-center sm:text-left space-y-2">
                                            <h3 className="text-3xl font-black italic uppercase italic">Rubel Maneiro</h3>
                            ...
                            <div className="space-y-6 flex-1 flex flex-col shrink-0">
                                ...
                                <div className={`p-8 rounded-[2rem] border relative group flex flex-col justify-between overflow-hidden shadow-2xl transition-all ${isDark ? 'bg-[#0D1117] border-[#30363D]' : 'bg-white/40 border-[#CBD5E1]'}`}>
                                    <pre className={`font-mono text-xs leading-[1.8] overflow-auto max-h-[150px] custom-scrollbar transition-all ${isDark ? 'text-emerald-400/70' : 'text-emerald-700'}`}>
                                        {activeMenu !== 'team' && (
                                            (activeTab === 'js' ? `fetch("${baseUrl}${currentEndpoint?.path}")\n  .then(res => res.json())\n  .then(console.log);` :
                                                activeTab === 'python' ? `import requests\nres = requests.get("${baseUrl}${currentEndpoint?.path}")\nprint(res.json())` :
                                                    `curl -G "${baseUrl}${currentEndpoint?.path}"`)
                                        )}
                                    </pre>
