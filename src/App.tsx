...
import { motion, AnimatePresence, Reorder, useMotionValue, useTransform, LazyMotion, domAnimation } from 'framer-motion';
...
function App() {
  ...
  const handlePan = (_: any, info: any) => {
    ...
    if (window.scrollY <= 0 && info.offset.y > 0) {
      pullY.set(Math.min(info.offset.y, pullThreshold + 20));
      setPullProgress(Math.min(info.offset.y / pullThreshold, 1));
    } else {
      pullY.set(0);
      setPullProgress(0);
    }
      ...
      <div className={`min-h-screen transition-colors duration-500 pb-20 ${focusedInput ? 'pb-80' : ''}`}>
        ...
        <main className="relative z-10 w-full overflow-hidden">
          ...
          <motion.div
            ...
            className="max-w-xl mx-auto px-6 pt-8 space-y-8"
          >
